...
Pc=0.8;  %GA crossover probability        
Pm=0.05; %GA mutation probability
...
y1 = value_calc_GDOP(x1,z1,N,AP_num,RP_loc,AP_init_num);  %calculate GDOP from function
for i=1:1:M/2  %
...
%%
    ...
    z3=double(z4).*n/PT;        
    y3 = value_calc_GDOP(x3,z3,N,AP_num,RP_loc,AP_init_num);
...
if G_E(t+1) == G_E(t)  
 ...
 x1;  
 z1;
y1 =  value_calc_GDOP(x1,z1,N,AP_num,RP_loc,AP_init_num);  %GDOP at new AP location
...
AP_loc = [ 2 14 8 x1(:,F);5 5 25 z1(:,F)];  
for i = 1:AP_total
    for j =1: 154
