function local_error = knn_algorithm_comp_a(RP_loc,power_rec_a,power_real_a,temp_user,K,U,select_APs)
N = length(RP_loc(:,1));
temp = ones(2,N);
...
for i = 1:N
    total_dis_a = 0;
    for j = 1:select_APs
    total_dis_a = total_dis_a + (power_rec_a(j,i) - power_real_a(j))^2;
    end
    temp(2,i) = total_dis_a; %%%%%%%%%%%%%
    temp(1,i) = i;
end
temp= temp';
